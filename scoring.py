...
CANDIDATE_SCORING_REFERENCE = {
    ...
    "product_fields": {
        "deal_title":        "On-page title (Playwright h1 / og:title / soup / JSON-LD) — display only.",
        "deal_price":        "First price-like string found (JSON-LD offers → semantic tags → regex).",
        "original_price":    "Strikethrough / compare-at / class patterns when present.",
        "brand":             "Brand name extracted from JSON-LD, microdata, or og:brand.",
        "image_url":         "Primary product image from JSON-LD, og:image, or itemprop=image.",
        "gtin":              "GTIN/EAN/UPC/ISBN from JSON-LD or microdata (empty if not found).",
        "completeness_score": "Fraction 0.0–1.0: how many of (title, price, image, brand) are present.",
        "canonical_key":     "Deterministic dedup key: 'gtin:<val>', 'bt:<hash>' (brand+title), or 't:<hash>' (title only).",
...
def build_candidate_records(analyses: list[dict]) -> list[dict]:
    ...
    for item in analyses:
        ...
        common = {
            ...
            "image_url": site.get("image_url", "") or "",
            "gtin": site.get("gtin", "") or "",
            "completeness_score": site.get("completeness_score", ""),
            "canonical_key": site.get("canonical_key", "") or "",
            "meh_signals": ms,
